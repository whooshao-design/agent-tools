...
// render each block to SVG/PNG, and write a results table (results.md, plus results.json for scripts).
//
...
// 围栏规则与 manage-feishu-doc 发布时一致：``` 或 ~~~（3 个以上）、最多缩进 3 格、info 第一个词是 mermaid；
// 其他代码块里的内容不算图
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fence) {
      if (new RegExp(`^ {0,3}${fence.char}{${fence.length},}\\s*$`).test(line)) {
        if (fence.mermaid) blocks.push({ id: `D${blocks.length + 1}`, heading: fence.heading, line: fence.line, code: fence.body.join('\n') });
        fence = null;
    ...
    if (open) fence = { char: open[2][0], length: open[2].length, indent: open[1].length, mermaid: (open[3] || '').toLowerCase() === 'mermaid', line: i + 1, heading, body: [] };
  }
  return blocks;
}
...
function main() {
  ...
  (async () => {
    ...
    // 供其他脚本解析（manage-feishu-doc 的 lint）
    fs.writeFileSync(path.join(outDir, 'results.json'), JSON.stringify(blocks.map((b) => ({ id: b.id, heading: b.heading, line: b.line, results: results[b.id] })), null, 2) + '\n');
    console.log(table);
