...
| ID / 内容指纹 | 严重度 | 类别 | 对象引用 | 问题 | 是否阻塞 | 状态 |
|---|---|---|---|---|---|---|
| TR-001 / <fingerprint> | Blocker / High / Medium / Low | | CHG-001 / DEC-001 / AC-001 / 章节 | | 是 / 否 | Open / Resolved / Accepted |
...
> 必须覆盖方案变更地图中的每个 `CHG-*`，且集合完全一致；`保持不变` 项按受保护的回归边界评审，不要求实现步骤。
...
| 检查项 | 结果 | 证据或 Finding |
|---|---|---|
| 一页结论、变更地图与权威正文一致 | Pass / Finding / Unknown | |
| 现状与新增/修改/删除/不变项分离 | | |
| 同级单一划分维度，DEC 唯一承载公共决策 | | |
| CHG 集合一致、粒度可独立实施和验证 | | |
| AC→CHG→验证/发布闭环，无孤儿变更 | | |

结构类 finding 标记：`层级错位` / `结构重叠` / `覆盖缺口` / `孤儿变更` / `重复事实源` / `表达冗余`。只有影响决策、实施或验证时才记录，不评价无影响的标题和个人文风。
