...
类别使用：`结构/可读性` / `需求追踪` / `可行性` / `架构` / `接口数据` / `可靠性` / `性能` / `安全` / `可观测性` / `可测试性` / `发布` / `证据`。
...
| 测试 | 结果 | 证据或 Finding |
|---|---|---|
| 读者测试：`reader-test/agent-v<N>.md`（统一比对报告，含两种读者）按 `reader-test-protocol.md` §5 判定——全文读者七问通过、截断读者前五问通过（Q6/Q7 允许"找不到"）、成立的探针问题已处置 | Pass / Finding / Unknown | |
