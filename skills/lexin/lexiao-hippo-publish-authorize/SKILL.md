...
参数：`--demand-id`（必填）、`--apps`（add 必填，逗号分隔）、`--namespace`（默认 `encryption`）、
`--instance-type`（默认 `1`）、`--resource-type`（默认 `hippo`）、`--profile`（默认
...
- **响应不得截断。** 完整读取响应体再解析；`slice`/`head`/`tail` 截断会切断 JSON，
  导致解析失败或误判成功。
- 登记范围按"本次需求建了配置的应用"取全集，宁可多登记也不要漏——漏登记会让该应用的配置
  在发布时被跳过，而多数配置缺失路径是静默降级、无任何告警信号。
...
- 返回 HTML 登录页或 `result_rows` 恒为空：登录态失效，用 `get-browser-session` 刷新同一 profile。
