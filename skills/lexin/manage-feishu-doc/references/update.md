...
片段文件按 `publish.md`「本地怎么写」的规则写：mermaid 建成文本绘图小组件，`> [!TIP]` 等变高亮块，图片路径相对片段文件所在目录。片段里的 `# 标题` 就是正文标题，不会被当成文档标题拿走。空片段报 `EMPTY_FRAGMENT`，要删除请用 `delete` 系列。
...
`edit` 需要 `docx:document`、`docx:document:readonly`、`docx:document:write_only`、`docs:document.comment:read`、`docs:document.media:upload`、`board:whiteboard:node:create`、`offline_access`；以 `auth-check --operation=edit` 为准。
