...
- **部署地址从 targets.json 读取默认值**：`/home/joney/projects/ai/agent-tools/skills/lexin/test-dubbo-api/targets.json` 存储各应用各环境的 IP:Port，优先使用默认配置
...
| 输入环境 | 线路 | 服务模拟器 URL | `env` 参数值 |
|------|------|---------------|-------------|
| `test` / `测试` / `offline` / `线下` / `stable` | stable | `https://stable-bianque.lexinfintech.com` | `prj` |
| `pre` / `预发布` / `gray` / `灰度` / `prod` / `online` / `线上` / `生产` | pre | `https://bianque.lexinfintech.com` | `pre` |

固定脚本支持传入上述别名。`--env auto` 可从 Redis `queryRedis` 参数里的 key 右侧环境 token 推断线路；推断不到时必须让用户确认。
默认 `--transport auto`：pre 线路使用已登录扁鹊页面上下文发 `/serviceEmulator/request`，避免手工 Cookie 401；stable 线路优先走 HTTP 请求，失败时再回退浏览器上下文。内网域名会自动补充 `NO_PROXY`，避免请求走本地代理导致 CONNECT 502。

## IP:Port 默认配置

读取 `/home/joney/projects/ai/agent-tools/skills/lexin/test-dubbo-api/targets.json`：
...
```bash
python3 /home/joney/projects/ai/agent-tools/skills/lexin/test-dubbo-api/scripts/dubbo_scenario.py \
