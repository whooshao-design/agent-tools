...
这些表是查询 Hawk 相关流水时的候选表。使用前先确认实例，再查结构：DBA 使用 `DESCRIBE`，HTTP 使用 `SHOW CREATE TABLE`；不要只根据表名假设字段或业务口径。`DecisionWaterHawkCK` 默认走 `bastion_dba`，其他 CK 实例默认走 HTTP，用户显式指定通道优先。
...
- 用户只说“线上流水”“决策流水”“规则流水”时，先从上表选候选表，再按通道验证结构。
