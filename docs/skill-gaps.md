...
| 2026-09-24 | Claude Code 不删除会话的 `/tmp/claude-<uid>/<项目>/<session-id>/`（scratchpad 与后台任务输出），曾累积到 1.4G | `install.py --with-session-cleanup` 加 SessionEnd hook，会话结束时只删本会话目录 |
